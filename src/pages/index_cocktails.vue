...
<template>
  ...
  <div class="filtreBlock w-full flex justify-center mt-6" v-if="FilterPage">
    <div class="filterPage bg-white h-64 w-5/12 rounded-xl shadow-lg relative">
      ...
      <div
        class="w-full h-full flex -mt-6 flex-col items-center justify-around"
      >
        <div
          class="
            searchBar
        ...
        >
          ...
          <input
            type="text"
            placeholder="Ingredient (Ex: Lemon juice,Gin,Mint)"
            class="focus:outline-none w-4/6 sm:w-5/6 bg-white"
        ...
        <div class="w-full flex justify-around items-center">
          ...
          <div class="relative inline-flex">
            ...
            >
              ...
              <option v-for="glass in categories.glass" :key="glass">
                {{ glass.strGlass }}
              </option>
            </select>
          </div>
        ...
        <div class="searchButton">
          <button
            class="bg-red-400 text-white text-xl rounded-full px-8 py-1 shadow"
  ...
  <div class="w-full flex justify-center" :class="{ hidden: loading }">
    <div class="flex items-center mt-32" v-if="results == 'None'">
      <img
        src="https://distok.top/stickers/755240383084232756/755242982843613305.gif"
        alt=""
        class="w-48"
      />
    ...
    <div
      class="flex justify-around flex-wrap w-full 2sm:w-9/12 mt-14"
      v-else-if="randomDrink != null && results == null"
    >
      <h1 class="text-center font-bold text-xl">Random drinks</h1>
      <div class="flex justify-around flex-wrap mt-4">
        <foodCard
          v-for="result in randomDrink"
          :key="result"
          :result="result"
        ></foodCard>
      </div>
    ...
    <div
      class="flex flex-col w-full 2sm:w-9/12 mt-14"
      v-else-if="popularDrink != null && results == null"
    >
      <h1 class="text-center font-bold text-xl">Popular drinks</h1>
      <div class="flex justify-around flex-wrap mt-4">
        <foodCard
          v-for="result in popularDrink"
          :key="result"
          :result="result"
        ></foodCard>
      </div>
        ...
        .then((response) => {
          ...
          this.filterResults = this.results.filter((result) => {
            return (
              result.strCategory
                .toLowerCase()
                .includes(this.categoryFiltre.toLowerCase()) &&
              result.strGlass
                .toLowerCase()
                .includes(this.glassFiltre.toLowerCase()) &&
              (result.strAlcoholic
                .toLowerCase()
                .includes(alcoholic.toLowerCase()) ||
                result.strAlcoholic === "Optional alcohol")
            );
          });
                ...
                if (response.data.drinks !== "None Found") {
                  ...
                  this.filterResults.forEach((result) => {
                    response.data.drinks.filter((resResult) => {
                      if (
                        resResult.idDrink.toLowerCase() ==
                        result.idDrink.toLowerCase()
                      ) {
